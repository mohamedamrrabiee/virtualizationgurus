...
Because a shared NSX Manager serves multiple domains, its version has to stay compatible with every attached vCenter -- and that compatibility isn't binary. The wizard documents it as a state table:
...
| Scalability | Scales independently | Constrained by shared instance headroom |
...
Whichever NSX topology you pick, it's a separate decision from how the domain reaches the outside world. Centralized Connectivity keeps north-south traffic funneled through a dedicated Edge cluster (and only becomes VPC-ready once that Edge cluster's Tier-0 is deployed in active-standby HA), while Distributed Connectivity is VPC-ready immediately but needs a Virtual Network Appliance cluster if you want stateful NAT or load balancing. Shared-vs-dedicated NSX and centralized-vs-distributed connectivity are two independent switches you flip for every domain -- not one combined choice.
...
Next in this series: vSAN ESA vs OSA -- the storage architecture decision that shapes hardware sizing before you ever get to networking.
